...
import { makeStore, lineLabel, sameRate, backdateNeedsOk } from './store.js?v=233';

const $ = id => document.getElementById(id);

/* ── Ошибки, которые видят ЛЮДИ ──────────────────────────────────────────
   Дарина 07.08: «щоб при будь-яких помилках ми усе знали і швидко фіксили».
   До этого в программе не было НИ ОДНОГО обработчика: у Алёны падало — она
   видела «не работает», а мы не узнавали никогда, пока она не напишет.

   Что уходит в базу: текст ошибки, где случилось, версия, экран, браузер.
   Чего НЕ уходит — и это главное: ни сумм, ни ФИО, ни содержимого полей.
   Ошибка может случиться посреди ввода зарплаты, и «подробности» утащили бы
   деньги в таблицу, которую читают иначе, чем деньги. Режет длину и считает
   частоту сама база (log_client_error), клиенту тут доверия нет.

   ⚠ Ничего не показываем человеку. Это фон: у него уже что-то сломалось, и
   второе красное окно поверх не поможет. Тосты об ошибках, которые он ДОЛЖЕН
   видеть, показывают сами обработчики — этот слой ловит то, что мимо них. */
{
  ...
  window.addEventListener('error', e => {
    ...
    // нет message — от них толку ноль, а место занимают.
    if (!e.message) return;
...
/* Дарина 12.08: «там ще два значка завантаження крутяться, коли натискаєш
   увійти». Так и было: кнопка «Войти» рисует свой кружок, и сверху появлялась
   эта полоска со вторым. На экране входа полоска не нужна вовсе — кнопка и так
   всё говорит, а два кружка на одно действие выглядят как поломка. */
const naEkraneVhoda = () => {
  const s = $('loginStage');
  return !!s && s.offsetParent !== null;
};
function netStart(действие) {
  // ⚠ Возвращает, СЧИТАЛИ ли мы этот запрос. netEnd надо звать только если да —
  // иначе счётчик уйдёт в минус и полоска погаснет посреди чужого запроса.
  if (naEkraneVhoda()) return false;                  // на входе молчим
  if (++netPending > 1) return true;                  // уже показываем
  netStarted = Date.now();
  netClearTimers();
  // И слово по делу: «Сохраняем» на чтении вводило в заблуждение — при входе
  // и при открытии экрана ничего не сохраняется. Читающие методы отличаем тем
  // же признаком, что и для повторов.
...
/* ── Повторяем, а не сдаёмся ─────────────────────────────────────────────
   Дарина 11.08: «треба щоб навіть якщо щось повільне і тд, воно всеодно
   продовжувало пробувати поки не завантажить. бо це капець, коли взагалі не
   відкривається». В тот день «Расчёт» не открывался ни у неё, ни у СЕО — ни с
   телефона, ни с компьютера.

   В error_log за 11.08 десяток записей подряд, на каждой версии от 174 до 178:
     listPayroll: canceling statement due to statement timeout   8226 мс

   Это не наш сбой и не обрыв связи. Это база сама убивает запрос, который идёт
   дольше восьми секунд (предел роли authenticated в Supabase). Тот же запрос с
   быстрой машины идёт 1,1 секунды — значит он НА ГРАНИ, и когда заходят
   несколько человек разом, границу переходит. В журнале это и видно: иногда
   успевает (5,2 с), иногда нет (8,2 с). Ровно тот случай, когда повтор спасает.

   ⚠ ПОВТОРЯЕМ ТОЛЬКО ЧТЕНИЕ, и это запрет, а не осторожность. Повторить
   payoutGive — значит выдать человеку деньги дважды. Список разрешённого —
   БЕЛЫЙ: list*, get*, ping. Появится завтра новый метод — он по умолчанию НЕ
   повторяется. Это правильная сторона ошибки.

   ⚠ И ТОЛЬКО ТЕ ОШИБКИ, которые проходят сами: таймаут базы, обрыв связи,
   отказ шлюза. «Нет прав» или «неверные данные» повторять бессмысленно: ответ
   будет тот же, а человек прождёт минуту вместо мгновенного «нельзя».

   Паузы растут — 1, 2, 4, 8, 15, 15 секунд. Растут намеренно: база и так не
   справляется, долбить её раз в секунду значит мешать ей подняться. Всего около
   45 секунд, потом отдаём ошибку. Крутить колесо бесконечно хуже, чем честно
   сказать «не вышло»: человек хотя бы поймёт, что ждать больше нечего. */
...
function стоитПовторить(e) {
  ...
  // «canceling statement» — текст самой базы; остальное — сеть и шлюзы.
...
async function повторяяЧтение(имя, первый, ещёРаз) {
  if (!ЧИТАЮЩИЕ.test(имя)) return первый;        // запись — одна попытка, и всё
  const начало = performance.now();
  let попытка = 0, ответ = первый;
  for (;;) {
    try {
      const r = await ответ;
      // Получилось не с первого раза — это надо знать: значит база на грани.
      ...
      netClearTimers();          // иначе таймеры полоски перебьют наше сообщение
...
/* Права на карточку РАЗНЫЕ на трёх уровнях, и это не описка, а три отдельные
   ...
   показали Алёне, а emp_insert ей не открывали — она бы жала и получала отказ.
     · правка полей и архив — owner, ceo, operator, cashier1 (emp_update, 132);
     · ЗАВЕДЕНИЕ карточки   — owner, ceo, cashier1 (emp_insert, 131): паспорт,
       должность и оклад нового человека приходят к бухгалтеру первыми, и
       заводить карточку, не умея исправить в ней опечатку, бессмысленно —
       поэтому 131 и 132 сделаны в один день;
     · СТАВКИ уже заведённого — owner, ceo (reconcile_employee_rates, 081):
       правка ставки задним числом суммируется за месяц и даёт двойную оплату.
       Завести строку начисления ВМЕСТЕ с новой карточкой бухгалтер может.
   ⚠ Норму часов и тайну ЗП стережёт колоночный сторож (111/132), а не эти
   хелперы: они про экран, сторож — про базу.
...
async function enter() {
  ...
  // ⚠ Подсказка обязана совпадать с тем, что человеку РЕАЛЬНО можно, — и это
  //    уже дважды расходилось. Сначала она обещала правку тем, кто править не
  //    мог; теперь наоборот: бухгалтер заводит карточки (131/132), а текст всё
  //    ещё отправлял её за этим к владельцу. Условие смотрит на КАЖДОЕ право
  //    отдельно, а не на одно за все.
  $('roNote').innerHTML =
    canEditRates() ? ''
...
function factClass(c) {                                             // класс фона клетки по факту
  ...
  /* ⚠ Прошедший рабочий день БЕЗ отметки — это НЕ «вышел по плану».
     До августа было так: месяц считался по плану, и зелёная заливка честно
     означала «отработал как назначено». С 01.08 в «Осталось выдать» идут только
     отмеченные дни (calc_rule.fact_required_from), а цвет остался прежним — и
     график стал говорить обратное расчёту. На 18.08: из 530 прошедших рабочих
     дней зелёными выглядели все 530, отмечено было ШЕСТЬ, а 3 228 501 ₽ висели
     неподтверждёнными. Найти незаполненное глазами было нельзя вовсе.
     Теперь такой день — рамка без заливки: «намечено, но не подтверждено».
     Дарина 19.08: «відміть усі автозалиті комірки рамочкою замість заливки». */
...
/* У кого смена оплачивается СУММОЙ, а не часами: ставка «за смену» (сутки, 12ч,
   почасово). Таким людям иногда нужен день с ДРУГОЙ суммой — половина смены,
   подмена, — и вписать её было негде: режим суммы в клетке включён только
   санитаркам (isAmountCell), остальным суммы попадали лишь импортом.

   Живой случай, с которого началось: у Руфиной (медсестра, ставка «12ч = 5070»)
   7 августа стоит полсмены на 6 часов. Вида «6 часов» в её ставке нет, маршрут
   пустой — и день платит 2 535 ₽ ТОЛЬКО потому, что сумма лежит в клетке из
   импорта. Убрать её значило бы обнулить день, а вписать заново Алёна не могла:
   «не понимаю как 5070 поставить, у меня часы получаются только».

   Почему не через клик, как у санитарок: клик по клетке — это «вышел / не вышел»,
   им отмечают выходы десятками. Забрать его ради редкого случая значит сломать
   ежедневную работу. Поэтому поле живёт в окне смены, под карандашом. */
...
function scheduleCellPopup(empId, day, pos = 'main') {
  ...
  $('scSave').onclick = async () => {
    ...
    /* Своя сумма — только если поле показано (ставка за смену). Ключ кладём в
       объект ЛИШЬ когда поле есть: setScheduleCell смотрит `'amount_kop' in cell`,
       и безусловный null стирал бы суммы там, где поле не показывали. */
...
function drawOverview() {
  ...
  const bento = `<div class="ov-bento">`
    ...
    + (hasMarked && (forecast > salary || ovFcast.sum)
        ...
        : '')
    /* Выданное — в разрезе, парой. Дарина 18.08: «є офіційно на карту, це ж
       видано; якщо ж окремо скільки видано, то можна вказати і скільки видано
       готівкою». Обе плитки — слагаемые строки «Выдано» выше, и суммы там те же. */
    /* Ровно card, как строка «· на карту» в разбивке. Складывать сюда ещё и
       расчёт при увольнении нельзя: в разбивке он отдельной строкой, и плитка
       разошлась бы с ней — та самая ловушка «одна подпись, два числа». */
    + metric('Выдано на карту', rub(card) + ' ₽', '', 'rgba(62,115,216,.34)')
    + metric('Выдано наличными', rub(cash) + ' ₽', '', 'rgba(31,165,101,.4)')
    /* Касса — только когда ей ПОЛЬЗУЮТСЯ. Таблица payout пуста за всю историю,
       и плитка «Проведено кассой» стояла вечным нулём: место занимает, а
       сказать ей нечего. Появится сама, как только проведут первую выдачу. */
